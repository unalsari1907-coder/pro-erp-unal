// lib/screens/yeni_siparis_screen.dart
...
class YeniSiparisScreen extends StatefulWidget {
  ...
  @override
  State<YeniSiparisScreen> createState() => _YeniSiparisScreenState();
}

class _YeniSiparisScreenState extends State<YeniSiparisScreen> {
  final TextEditingController _aramaController = TextEditingController();
  final TextEditingController _siparisNoController = TextEditingController();
  ...
  Future<void> _ilkVerileriYukle() async {
    ...
    try {
      ...
      _mesaj(
        'Sipariş ekranı verileri yüklenemedi: $e',
        Colors.red,
      );
    }
  }
  ...
  double _satirBrut(Map<String, dynamic> item) {
    return _sayi(item['miktar']) * _sayi(item['birim_fiyat']);
  }

  double _satirIndirim(Map<String, dynamic> item) {
    return _satirBrut(item) * _sayi(item['indirim']) / 100;
  }
  ...
  double _satirKdv(Map<String, dynamic> item) {
    return _satirMatrah(item) * _sayi(item['kdv_orani']) / 100;
  }
  ...
  Future<void> _terminTarihiSec() async {
    final secilen = await showDatePicker(
      context: context,
      initialDate:
          _terminTarihi ?? DateTime.now().add(const Duration(days: 1)),
  ...
  String _kisaTarih(DateTime? tarih) {
    if (tarih == null) return 'Termin Tarihi';
  ...
  Future<void> _siparisiKaydet() async {
    ...
    try {
      ...
      final response = await SupabaseService.supabase.rpc(
        'satis_siparis_olustur',
        params: {
          'p_siparis_no': _siparisNoController.text.trim(),
          'p_cari_id': _cariId,
          'p_depo_id': _depoId,
          'p_fiyat_tipi': _fiyatTipi,
          'p_odeme_tipi': _odemeTipi,
          'p_termin_tarihi':
              _terminTarihi?.toIso8601String().split('T').first,
      ...
      if (siparisId == null) {
        throw Exception(
          'Sipariş fonksiyonu geçerli bir sipariş ID döndürmedi.',
        );
      }
      ...
      _mesaj(
        'Sipariş başarıyla kaydedildi. Sipariş ID: $siparisId',
        Colors.green,
      );
        ...
        child: Wrap(
          ...
          children: [
            ...
            SizedBox(
              width: 200,
              child: TextField(
                controller: _siparisNoController,
                decoration: const InputDecoration(
    ...
    return BelgeAltToplamCubugu(
      ...
      actions: [
        ElevatedButton.icon(
          ...
          label: Text(
            _kaydediliyor ? 'Kaydediliyor...' : 'Siparişi Kaydet',
          ),
        ),
      ],
    );
