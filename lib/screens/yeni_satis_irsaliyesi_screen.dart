// lib/screens/yeni_satis_irsaliyesi_screen.dart
...
class YeniSatisIrsaliyesiScreen extends StatefulWidget {
  ...
  @override
  State<YeniSatisIrsaliyesiScreen> createState() =>
      _YeniSatisIrsaliyesiScreenState();
}

class _YeniSatisIrsaliyesiScreenState extends State<YeniSatisIrsaliyesiScreen> {
  ...
  Future<void> _duzenlenecekIrsaliyeyiYukle() async {
    ...
    if (irsaliye == null || irsaliyeId <= 0) {
      _mesaj('Düzenlenecek satış irsaliyesi bulunamadı.', Colors.red);
      return;
    }
    ...
    try {
      ...
      for (final detay in detaylar) {
        ...
        final mevcutFiyat = _sayi(detay['birim_fiyat']);

        String fiyatTipi = 'MAN';

        if ((mevcutFiyat - stok.satisFiyatiPerakende).abs() < 0.001) {
          fiyatTipi = 'SFP';
        } else if ((mevcutFiyat - stok.satisFiyatiToptan).abs() < 0.001) {
          fiyatTipi = 'SFT';
        } else if ((mevcutFiyat - stok.satisFiyatiListe).abs() < 0.001) {
          fiyatTipi = 'SFL';
        } else if ((mevcutFiyat - stok.alisFiyati).abs() < 0.001) {
          fiyatTipi = 'AFN';
        }
  ...
  void _sepeteEkle(StokModel stok, {String fiyatTipi = 'SFP'}) {
    final index = _sepet.indexWhere((item) => item['stok_id'] == stok.stokId);

    final fiyat = _fiyatDegeri(stok, fiyatTipi);
  ...
  double _fiyatDegeri(StokModel stok, String fiyatTipi) {
    switch (fiyatTipi) {
      case 'AFN':
      ...
      case 'SFL':
        return FiyatSeridi.fiyatDegeri(stok, 'SFL', sflNetOlarak: true);
      case 'SFP':
      default:
  ...
  Future<void> _fiyatTipiSec(int index) async {
    ...
    final secilen = await showDialog<String>(
      ...
      builder: (dialogContext) {
        return SimpleDialog(
          title: const Text('Fiyat Tipi Seç'),
          children: [
            _fiyatSecimSatiri(
              dialogContext,
              'AFN',
            ...
            _fiyatSecimSatiri(
              dialogContext,
              'SFL',
              stok.satisFiyatiListe,
              Colors.purple,
            ),
            const Divider(),
            SimpleDialogOption(
              onPressed: () {
                Navigator.pop(dialogContext, 'MANUEL');
              },
              child: const Row(
                children: [
                  SizedBox(
                    width: 50,
                    child: Icon(Icons.edit_rounded, color: Colors.blueGrey),
                  ),
                  Text(
                    'Manuel Fiyat Gir',
    ...
    if (secilen == 'MANUEL') {
      await _manuelFiyatGir(index);
      return;
    }

    setState(() {
      _sepet[index]['fiyat_tipi'] = secilen;
      _sepet[index]['birim_fiyat'] = _fiyatDegeri(stok, secilen);
    });
  }

  Future<void> _manuelFiyatGir(int index) async {
    final item = _sepet[index];
            ...
            child: TextField(
              ...
              onSubmitted: (_) {
                ...
                if (fiyat != null && fiyat >= 0) {
                  Navigator.pop(dialogContext, fiyat);
                }
              },
            ),
          ),
    ...
    setState(() {
      _sepet[index]['fiyat_tipi'] = 'MAN';
      _sepet[index]['birim_fiyat'] = sonuc;
    });
  }

  Widget _fiyatSecimSatiri(
    ...
    return SimpleDialogOption(
      onPressed: () {
        Navigator.pop(dialogContext, kod);
      },
      child: Row(
        children: [
          SizedBox(
  ...
  Future<void> _kaydet() async {
    ...
    if (_cariId == null || _depoId == null || _sepet.isEmpty) {
      _mesaj('Cari, depo ve ürün seçmelisiniz.', Colors.orange);
      return;
    }
    ...
    try {
      ...
      final sonuc = await SupabaseService.supabase.rpc(
        _duzenlemeMi ? 'satis_irsaliye_guncelle' : 'satis_irsaliye_olustur',
        params: _duzenlemeMi
            ? {
      ...
      _mesaj(
        _duzenlemeMi
            ? 'Satış irsaliyesi güncellendi.'
            : 'Satış irsaliyesi oluşturuldu: $sonuc',
        Colors.green,
      );
  ...
  Widget _urunKarti(StokModel stok) {
    return BelgeStokAramaKarti(
      ...
      onFiyatSecildi: (kod) => _sepeteEkle(stok, fiyatTipi: kod),
    );
  }
  ...
  void _miktarArtir(int index) {
    ...
    if (mevcut >= stok.stokMiktari) {
      _mesaj(
        'Miktar mevcut stoğu aşamaz. Mevcut stok: '
        ...
        return AlertDialog(
          ...
          content: TextField(
            ...
            decoration: InputDecoration(
              labelText: 'Miktar',
              helperText: 'Mevcut stok: ${stok.stokMiktari.toStringAsFixed(0)}',
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text(
          _duzenlemeMi
              ? 'SATIŞ İRSALİYESİ DÜZELT'
              : (widget.konsinye
                    ? 'KONSİNYE ÇIKIŞ İRSALİYESİ'
                    : 'YENİ SATIŞ İRSALİYESİ'),
          style: const TextStyle(fontWeight: FontWeight.bold),
        ),
      ),
      body: _yukleniyor
